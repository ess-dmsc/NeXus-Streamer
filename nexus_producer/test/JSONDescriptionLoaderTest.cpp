...
TEST(JSONDescriptionLoaderTest, isValidJson_reports_false_for_empty_string) {
  ...
  ASSERT_THROW(JSONDescriptionLoader::checkIsValidJson(emptyString), nlohmann::json::parse_error);
}
...
TEST(JSONDescriptionLoaderTest, isValidJson_reports_false_for_invalid_json) {
  ...
  ASSERT_THROW(JSONDescriptionLoader::checkIsValidJson(invalidJson), nlohmann::json::parse_error);
}
