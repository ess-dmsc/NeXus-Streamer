...
struct RunInfo FLATBUFFERS_FINAL_CLASS : private flatbuffers::Table {
  ...
  uint64_t stream_offset() const { return GetField<uint64_t>(VT_STREAM_OFFSET, 0); }
  bool Verify(flatbuffers::Verifier &verifier) const {
    return VerifyTableStart(verifier) &&
           ...
           VerifyField<uint64_t>(verifier, VT_STREAM_OFFSET) &&
           verifier.EndTable();
  }
};
...
struct RunInfoBuilder {
  ...
  void add_stream_offset(uint64_t stream_offset) { fbb_.AddElement<uint64_t>(RunInfo::VT_STREAM_OFFSET, stream_offset, 0); }
