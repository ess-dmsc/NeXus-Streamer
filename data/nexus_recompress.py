    ...
    with h5py.File(original_file, 'r') as f_read:
        for dataset in datasets:
            ...
            with h5py.File(snappy_file, 'r+') as f_write:
                ...
                try:
                    # d_set = f_write.create_dataset(dataset, data_1[...].shape, compression="gzip", compression_opts=1)
...
if __name__ == '__main__':
    ...
    print str(timeit.timeit("write_file('SANS_test_blosc.hdf5', compress_type=32001)", number=10,
                        setup="from __main__ import write_file")) + "s for 10 repetitions"
    print str(os.path.getsize('SANS_test_blosc.hdf5')) + "bytes"
    print "timing with gzip level 1:"
    print str(timeit.timeit("write_file('SANS_test_gzip.hdf5', compress_type='gzip', compress_opts=1)", number=10,
                        setup="from __main__ import write_file")) + "s for 10 repetitions"
    print str(os.path.getsize('SANS_test_gzip.hdf5')) + "bytes"
    print "timing with no compression:"
    print str(timeit.timeit("write_file('SANS_test_noComp.hdf5', compress_type=None)", number=10,
                        setup="from __main__ import write_file")) + "s for 10 repetitions"
    print str(os.path.getsize('SANS_test_noComp.hdf5')) + "bytes"
